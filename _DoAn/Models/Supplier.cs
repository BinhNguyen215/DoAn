...
namespace _DoAn.Models
{
    public class Supplier
    {
        ...
        public DataTable GetSuplierData()
        {
            ...
            string sqlQuery = "select Supplier_id, SupplierName, Address, PhoneNumber, Email from Supplier";
            return connect.GetData(sqlQuery);
        }
        public bool AddSuplier(string name, string add, string phone, string email)
        {
            SqlCommand cmd = new SqlCommand("INSERT INTO Supplier (SupplierName, Address,PhoneNumber,Email) VALUES (@name, @add, @phone, @email)");
        ...
        public bool UpdateSuplier(string id, string name, string add, string phone, string email)
        {
            ...
            SqlCommand cmd = new SqlCommand("UPDATE	Supplier SET SupplierName = @name, Address = @add, PhoneNumber= @phone, Email = @email WHERE Supplier_id = @id");
            cmd.Parameters.AddWithValue("@name", name);
            cmd.Parameters.AddWithValue("@add", add);
            cmd.Parameters.AddWithValue("@phone", phone);
            cmd.Parameters.AddWithValue("@email", email);
